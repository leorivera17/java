...
public class Spider implements Entity, Move, Position, Execute, Schedule, AnimationPeriod {
    ...
    public Spider(String id, Point position, List<PImage> images, int resourceLimit, int resourceCount,
    ...
    public void executeActivity(WorldModel world, ImageStore imageStore, EventScheduler scheduler) {
        Optional<Entity> Target = world.findNearest(this.position, new ArrayList<>(List.of(WarpedTree.class)));

        if (Target.isPresent()) {
            Point tgtPos = Target.get().getPosition();

            if (this.moveTo(world, Target.get(), scheduler)) {
                Egg egg = tgtPos.createEgg(EGG_KEY + "_" + Target.get().getId(),
                        imageStore.getImageList(EGG_KEY), 0);
